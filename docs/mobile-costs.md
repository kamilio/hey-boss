...
The previous 1 GB volume was removed on 2026-09-21 after migrating its state and verifying recovery across a relay restart; no Fly volume remains. It previously cost $0.15/month even while detached. Five legacy snapshots totaling 168 MiB had five-day retention at deletion, so historical user data may remain in those snapshots until 2026-09-26. [Fly documents](https://fly.io/docs/volumes/snapshots/) that existing snapshots are removed only when their retention periods end; the CLI and documented Machines API offer no immediate snapshot deletion. Snapshot storage is $0.08/GB-month after the organization's first 10 GB of snapshots. No new user-data snapshots are created. Build machines/build services can contribute deployment-time charges; the estimate above covers the running app, not unrelated builders or other apps. Fly's dashboard **Billing / Cost Explorer** is the source for the actual organization invoice and whether granular or older bandwidth pricing applies. An older bandwidth plan may include allowances and uses its own rates; no plan switch is performed here.
