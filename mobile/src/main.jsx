...
function App(){
 ...
 return <Theme appearance={appearance} accentColor="indigo" grayColor="slate" radius="large"><div className="shell" ref={shellRef}>
  ...
  {paired&&<nav aria-label="Main navigation">{glassNav(<div className="tabs">{[['inbox','Inbox',Inbox],['history','Activity',History],['issues','Issues',ListTodo]].map(([value,label,Icon])=><button key={value} className={tab===value?'selected':''} aria-current={tab===value?'page':undefined} onClick={()=>{setTab(value);setNotice('');}}><Icon size={21} strokeWidth={1.8}/><span>{label}</span>{value==='inbox'&&pending.length>0&&<b>{pending.length}</b>}</button>)}<a href="/agents" className="agent-tab"><span>Agents</span></a><a href="/artifacts" className="artifact-tab"><span>Artifacts</span></a></div>)}</nav>}
