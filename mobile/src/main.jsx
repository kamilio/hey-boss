...
// Preserve existing bookmarks and links copied from the regular issue browser.
if(location.hash==='#issues'||new URLSearchParams(location.hash.slice(1)).has('issue')||new URLSearchParams(location.hash.slice(1)).get('view')==='issues')location.replace('/issues'+(location.hash==='#issues'?'':location.hash));
...
function App(){
 ...
 async function clearInbox(){
  ...
  try{const value=await api('/tasks/clear',{taskIDs:clearSnapshot.map(t=>t.taskID)});setClearSnapshot(null);setNotice(value.cleared+' notices moved to notification history.');await refresh();}
 ...
 function decisionLink(task){if(!decision(task)||task.status!=='pending'||!safeLink(task.linkURL))return null;let host;try{host=new URL(task.linkURL).hostname;}catch{return null;}return <div className="decision-link">{link(task)}<small>{host} · Opens in a new tab</small><p>Finish on the sign-in page, then return here to confirm.</p></div>;}
 ...
 return <Theme appearance={appearance} accentColor="indigo" grayColor="slate" radius="large"><div className="shell" ref={shellRef}>
  ...
  <main>
   ...
   <div className="page-title"><h1 id="page-heading" tabIndex={-1}>{tab==='settings'?'Settings':'Inbox'}</h1>{tab!=='settings'&&tab!=='issues'&&<div className="title-actions">{tab==='inbox'&&<><span className="count">{pending.length}</span><button className="clear-inbox-button" disabled={!pending.length||clearing} onClick={()=>{setClearError('');setClearSnapshot(pending);}}><CheckCheck size={16}/>Clear all</button></>}<button className="refresh-button" aria-label="Refresh inbox" disabled={pull.loading} onClick={pull.run}><RefreshCw size={18}/></button></div>}</div>
   {tab!=='settings'&&<><div className="segmented inbox-segments" role="group" aria-label="Inbox view"><button aria-pressed={tab==='inbox'} onClick={()=>setTab('inbox')}>Unread {pending.length>0&&<span>{pending.length}</span>}</button><button aria-pressed={tab==='history'} onClick={()=>setTab('history')}>History</button></div><p className="inbox-help fine">{tab==='history'?'Notification history — read updates, answers, and dismissed notices.':'Updates and questions waiting for you. Track project work in Issues.'}</p></>}
   {tab==='settings'?<section className="settings glass">
    ...
    {pending.length===0?<section className="empty glass"><span className="empty-symbol"><Check size={28}/></span><h2>You’re up to date</h2>{!state.pushEnabled&&<Button variant="soft" onClick={()=>setTab('settings')}>Enable notifications</Button>}</section>:pending.map(task=><article className={'task-card glass '+(task.severity||'info')} key={task.taskID}>
     ...
     {decisionLink(task)}{task.kind==='prompt'?<div className="response"><Button variant="soft" size="3" onClick={()=>openTask(task)}>Answer <MessageCircle size={16}/></Button></div>:responses(task)}{!decision(task)&&safeLink(task.linkURL)&&<div className="notification-actions">{link(task)}</div>}
    </article>)}
   </div>}
   </>}
  ...
  {paired&&<nav aria-label="Main navigation">{glassNav(<div className="tabs"><button className={tab!=='settings'?'selected':''} aria-current={tab!=='settings'?'page':undefined} onClick={()=>{setTab('inbox');setNotice('');}}><Inbox size={21} strokeWidth={1.8}/><span>Inbox</span>{pending.length>0&&<b>{pending.length}</b>}</button><a href="/issues" className="issues-tab"><ListTodo size={21} strokeWidth={1.8}/><span>Issues</span></a></div>)}</nav>}
  <Dialog.Root open={clearSnapshot!==null} onOpenChange={open=>{if(!open&&!clearing)setClearSnapshot(null);}}><Dialog.Content className="clear-inbox-dialog" maxWidth="440px" onCloseAutoFocus={event=>{event.preventDefault();(document.querySelector(".clear-inbox-button:not(:disabled)")||document.getElementById("page-heading"))?.focus();}} onEscapeKeyDown={event=>{if(clearing)event.preventDefault();}} onPointerDownOutside={event=>{if(clearing)event.preventDefault();}}>
   ...
   <Dialog.Description>{clearSnapshot?.length} notices will move to notification history. Updates and alerts become read. Pending questions and reviews are cancelled without an answer or approval. History will be kept.</Dialog.Description>
