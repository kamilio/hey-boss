use crate::health::remote::hosts;
use crate::health::{Snapshot, Store, readable_bytes};
use clap::Subcommand;
use std::io;
...
#[derive(Subcommand)]
pub enum Action {
    /// Install in ~/.local/bin and migrate an enabled maintenance schedule.
    Install,
    /// Open the terminal dashboard with tabs for each machine.
    Open,
...
pub fn run_remote(host: &str, action: &Action) -> io::Result<()> {
    if matches!(action, Action::Open) {
        return crate::tui::run(Some(host));
    }
...
pub fn run(action: &Action) -> io::Result<()> {
    ...
    match action {
        Action::Install => {
            ...
            let destination = home.join(".local/bin/hey-harvester");
            crate::install::publish(&executable()?, &destination)?;
            if config.automatic {
                schedule_binary(&store, true, config.interval_seconds, &destination)?;
            }
            println!(
                "Installed {}. Hey Boss services remain running.",
                destination.display()
            );
            Ok(())
        }
        Action::Open => crate::tui::run(None),
...
// Compatibility invocations must register the standalone binary, never `hey-boss run`.
pub(crate) fn executable() -> io::Result<PathBuf> {
    let current = std::env::current_exe()?.canonicalize()?;
    if current
        .file_name()
        .is_some_and(|name| name == "hey-harvester")
    {
        return Ok(current);
    }
    let home = std::env::var_os("HOME")
        ...
        .unwrap_or_default();
    [
        current.with_file_name("hey-harvester"),
        home.join(".local/bin/hey-harvester"),
        PathBuf::from("/opt/homebrew/bin/hey-harvester"),
        PathBuf::from("/usr/local/bin/hey-harvester"),
    ]
    .into_iter()
    .find(|p| p.is_file())
    .ok_or_else(|| io::Error::other("Install hey-harvester before enabling scheduled maintenance"))
}
