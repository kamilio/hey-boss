    ...
    for (name, value) in [
        ...
        (
            ...
            if artifact_page {
                ...
                "default-src 'none'; script-src 'self'; style-src 'self'; style-src-attr 'unsafe-inline'; connect-src 'self'; img-src 'self' data: blob:; font-src 'self'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'"
            },
        ),
    ] {
...
fn route(request: &mut tiny_http::Request, app: &App) -> Result<(u16, &'static str, Vec<u8>)> {
    ...
    if request.method() == &Method::Get {
        ...
        let asset: Option<(&str, &[u8])> = match path.as_str() {
            ...
            "/attachments.js" => Some((
                "text/javascript; charset=utf-8",
                include_bytes!("web/attachments.js"),
            )),
            "/attachments.css" => Some((
                "text/css; charset=utf-8",
                include_bytes!("web/attachments.css"),
            )),
            "/artifacts.js" => Some((
                "text/javascript; charset=utf-8",
