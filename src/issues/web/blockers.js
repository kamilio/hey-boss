...
const IssueBlockers = (() => {
  ...
  function card(issue) {
    ...
    return `<div class="side-section issue-blockers"><h2 class="side-heading">Blocked by${icon("blocked")}</h2>${[...linked,...inherited].length ? `<ul class="blocker-list">${[...linked,...inherited].map(i=>`<li>${link(i)}<span class="muted-text">${i.deleted_at ? "Deleted" : i.state==="closed" ? "Closed" : i.source==="subtask" ? "Unfinished subtask" : "Unfinished issue"}</span>${linkedNumbers.has(i.number) ? `<button type="button" class="icon-button" data-remove-blocker="${i.number}" aria-label="Remove blocker #${i.number}">${icon("x")}</button>` : ""}</li>`).join("")}</ul>` : `<p>${issue.state==="blocked" ? "No linked issue. Review the blocker in comments." : "No blocking issues."}</p>`}<button type="button" class="button" data-add-blocker>${icon("plus")}Add blocker</button>${active.length ? `<p>${issue.manual_blocked ? "Resolve these issues, then reopen after resolving the manual blocker." : "Reopens automatically when all blocking issues are resolved."}</p>` : ""}</div>`;
  }
  ...
  function options() {
    ...
    $("#blocker-picker-results").innerHTML=choices.length ? choices.map(i=>`<button type="button" class="subtask-option" data-select-blocker="${i.number}" ${busy?"disabled":""}><span>#${i.number}</span><strong>${esc(i.title)}</strong><span>${i.state==="blocked"?"Blocked":"Open"}</span></button>`).join("") : '<p class="muted-text">No matching unfinished issues.</p>';
  }
  async function open() {
