...
(() => {
  ...
  function render(data) {
    ...
    $("overview").replaceChildren(...[[machines.filter(m=>m.state==="connected").length+" / "+machines.length,"Connected machines"],[busy+" / "+slots,"Active sessions / capacity"],[machines.reduce((n,m)=>n+(m.pending||0),0),"Changes waiting to sync"],[(data.conflicts||[]).length,"Changes needing review"]].map(([value,label])=>{const c=element("div","metric");c.append(element("strong","",String(value)),element("span","",label));return c;}));
    const cards = machines.map(m => {
      const card = element("article","machine"), heading=element("div","machine-header"), title=element("div");
      title.append(element("h3","machine-title",m.hostname||m.host),element("p","machine-subtitle",m.role==="controller"?"Controller · shared queue authority":`Agent · ${m.host}`));
      ...
      const meta=element("div","machine-meta");pair(meta,"Heartbeat",age(m.heartbeat));pair(meta,"Last sync",m.role==="controller"?"Local":age(m.last_sync));pair(meta,"Configuration",m.role==="controller"?"Authority":m.applied_revision===m.desired_revision?"Up to date":"Applying…");pair(meta,"Software",m.deployment||m.build?.match(/build ([a-f0-9]+)/)?.[1]||"Installed");card.append(meta);
      ...
      for(const w of m.workers||[]) {
        ...
        if(!active.length)block.append(element("p","empty",`${w.eligible||0} eligible · No active sessions`));
        const controls=element("div","controls");for(const [action,label] of [[w.config?.enabled?"pause":"resume",w.config?.enabled?"Pause":"Resume"],["restart","Restart"],["stop","Stop"]]){const b=element("button",action==="stop"?"danger":"",label);b.type="button";Object.assign(b.dataset,{host:m.host,worker:w.id,signal:action,focus:`${m.host}:${w.id}:${action}`});controls.append(b);}block.append(controls);
  ...
  (async()=>{try{const response=await fetch("/api/bootstrap");const data=await response.json();csrf=data.csrf;await refresh();const events=new EventSource("/api/fleet/events");const connected=()=>{$("live").textContent="● Live connection";$("live").className="connected";refresh();};events.addEventListener("connected",connected);events.onmessage=connected;events.onerror=()=>{$("live").textContent="Reconnecting…";$("live").className="";};}catch(e){$("error").textContent=e.message;$("error").hidden=false;}})();
