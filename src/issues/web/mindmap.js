...
(() => {
  ...
  const nodeName = (node) => `${node.project_id !== graph.project.id ? `${projects.find((p) => p.id === node.project_id)?.name || node.project_id} · ` : ""}${node.title}`;
  const allNodes = () => new Map([...graph.nodes, ...graph.external_nodes].map((node) => [node.id, node]));
  function relationships(node, nodes) {
    return graph.links.filter((l) => l.from === node.id || l.to === node.id).map((link) => {
      ...
      return `<li><span class="relation-kind">${esc(text)}</span> <a data-map-link href="${esc(mapUrl(other.project_id, other.id))}">${esc(nodeName(other))}</a>${link.description ? `<span class="description">— ${esc(link.description)}</span>` : ""}${link.automatic ? ' <span class="automatic">automatic</span>' : ""}</li>`;
    }).join("");
  }
  function render() {
    if (!graph) return;
    ...
    for (const node of graph.nodes) {
      const rel = graph.links.filter((l) => l.from === node.id || l.to === node.id);
  ...
  async function load({ refresh = false } = {}) {
    ...
    try {
      ...
      const value = await response.json(); if (!response.ok || !value.ok) throw new Error(value.error?.message || "Cannot load mindmap");
