...
(() => {
  ...
  function details(nodes, incidents, visible) {
    ...
    const html = `<div id="${esc(node.id)}"><h2 tabindex="-1">${esc(HeyBossMap.displayTitle(node))}</h2><div class="meta"><span class="badge">${esc(node.kind)}</span>${node.state ? `<span>${esc(node.state)}</span>` : ""}${node.alias ? `<code>${esc(node.alias)}</code>` : ""}${node.assignee ? `<span>Assigned to ${esc(assigneeName(node.assignee))}</span>` : ""}${node.available === false ? "<span>Resource unavailable</span>" : ""}</div>${resource}${node.has_body || node.body ? bodyContent(node) : ""}${topics}${rel ? `<section class="topic-relationships">${pager}<ul class="relationships" aria-label="Relationships for ${esc(HeyBossMap.displayTitle(node))}">${rel}</ul></section>` : ""}</div>`;
