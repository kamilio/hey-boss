...
(() => {
  ...
  function details(nodes, incidents, visible) {
    ...
    const automaticPr = node.kind === "pr" && node.automatic === true;
    const attachments = automaticPr ? `<p class="automatic-pr-note">Shown automatically from an issue attachment. To change its label, move it or attach files and artifacts, create an explicit PR node with <code>hey-boss mm pr URL</code> in this project.</p>` : `<section id="node-artifacts"></section><section id="node-attachments"></section>`;
    const html = `<div id="${esc(node.id)}"><h2 tabindex="-1">${esc(HeyBossMap.displayTitle(node))}</h2><div class="meta">${HeyBossMap.kindIcon(node.kind)}${node.state ? `<span>${esc(node.state)}</span>` : ""}${node.alias ? `<code>${esc(node.alias)}</code>` : ""}${node.assignee ? `<span>Assigned to ${esc(assigneeName(node.assignee))}</span>` : ""}${node.available === false ? "<span>Resource unavailable</span>" : ""}</div>${resource}${attachments}${issueMetadata(node)}${node.has_body || node.body ? bodyContent(node) : ""}${topics}${rel ? `<section class="topic-relationships">${pager}<ul class="relationships" aria-label="Relationships for ${esc(HeyBossMap.displayTitle(node))}">${rel}</ul></section>` : ""}</div>`;
