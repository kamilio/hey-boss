...
const IssueSubtasks = (() => {
  ...
  function options() {
    ...
    target.innerHTML=matches.length ? matches.map(i=>{
      ...
      const reason=i.number===picker.parent.number ? "Current issue" : picker.ancestors.has(i.number) ? "Parent issue" : i.parent ? `Subtask of #${i.parent.number}` : i.state==="blocked" ? "Blocked" : i.state==="closed" ? "Closed" : "Open";
      return `<button type="button" class="subtask-option" data-existing-subtask="${i.number}" ${unavailable || busy ? "disabled" : ""}><span class="issue-state ${i.state}">${icon(i.state==="blocked"?"blocked":i.state==="closed"?"closed":"issue")}</span><span><strong><small>#${i.number}</small> ${esc(i.title)}</strong><span>${reason}</span></span>${unavailable ? "" : icon("plus")}</button>`;
