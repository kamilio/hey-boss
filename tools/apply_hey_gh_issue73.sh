...
patch_file="$(cd "$(dirname "$0")/../patches/hey-gh" && pwd)/issue73-read-deadline.patch"
source_dir="$(cd "$1" && pwd)"
...
if patch --directory "$source_dir" --batch --forward --dry-run -p1 < "$patch_file" >/dev/null 2>&1; then
  ...
  echo "Issue 73 patch already applied."
else
  echo "hey-gh source differs from the issue 73 baseline; no files changed." >&2
  exit 1
fi
