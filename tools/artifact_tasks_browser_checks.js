async page => {
  ...
  await page.evaluate(async () => {
    ...
    for (const issue of list.issues) {
      if (/^(Visual |Quick plan task)/.test(issue.title) && !issue.deleted_at) await action({action:'delete',number:issue.number,force:false});
    }
  });
  ...
  await page.getByRole('link',{name:'Visual plan task',exact:true}).click();
  await page.locator('[data-edit]').click();
  check(await page.locator('#editor-kind').inputValue() === 'plan', 'Editing restores task intent');
  ...
  await page.locator('#editor-kind').selectOption('implement');
  await page.locator('#editor-cancel').click();
  await page.locator('[data-edit]').click();
  check(await page.locator('#editor-kind').inputValue() === 'implement', 'Unsaved task selection survives closing and reopening');
  ...
  await page.locator('#quick-issue-title').fill('Quick plan task');
  ...
  check(await page.locator('#quick-issue-kind').inputValue() === 'plan', 'Quick Add preserves task selection after failure');
  check(await page.locator('#quick-issue-title').inputValue() === 'Quick plan task', 'Quick Add preserves the title after failure');
  await page.locator('#quick-issue-submit').click(); await page.locator('#quick-issue-dialog').waitFor({state:'hidden'});
  const retries = requests.filter(r => r.operation.action === 'create' && r.operation.title === 'Quick plan task');
  check(retries.length === 2 && retries[0].request_id === retries[1].request_id, 'Quick Add safely reuses the request ID');
  check(retries.every(r => JSON.stringify(r.operation.labels) === '["task:plan"]'), 'Quick Add sends artifact task metadata on every retry');
  ...
  for (const theme of ['light','dark']) {
    ...
    for (const width of [1440,768,390,320]) {
      ...
      await page.locator('#new-issue').click(); await page.locator('#editor-kind').selectOption('plan');
      await page.locator('#editor-subject').fill('Plan task with a longer title');
      ...
      check(await page.locator('#editor-kind-help').textContent() === 'A concrete plan, saved as a linked artifact.', `${theme}/${width}: contextual task help`);
