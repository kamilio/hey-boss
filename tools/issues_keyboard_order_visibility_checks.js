...
async page => {
  ...
  for(let move=1;move<=15;move++){
    ...
    check(await handle.evaluate(el=>{const r=el.getBoundingClientRect();return el===document.activeElement&&r.top>=-1&&r.bottom<=innerHeight+1&&el.contains(document.elementFromPoint(r.x+r.width/2,r.y+r.height/2))}),'Moved issue stays focused and visible after step '+move);
  }
