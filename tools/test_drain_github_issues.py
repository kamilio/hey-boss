...
class DrainTests(unittest.TestCase):
    ...
    def test_final_destination_check_prevents_deletion(self):
        github, dest = self.setup_move()
        dest.verify.side_effect = [None, RuntimeError("saved copy was edited")]
        with self.assertRaisesRegex(RuntimeError, "saved copy was edited"):
            drain.drain_one(github, dest, {"number": 7}, "boss", "open")
...
''')
    ...
    def test_native_cli_rejects_shared_request_id_without_writes(self):
        root = self.fake_github()
        result = self.cli("--request-id", "one-id-for-many-issues")
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("omit --request-id", result.stdout + result.stderr)
        self.assertFalse((root / "delete.called").exists())
        self.assertEqual(self.dest.call(["list"])["issues"], [])
